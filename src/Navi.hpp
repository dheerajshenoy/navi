...
public:
	explicit Navi()
	: KDDockWidgets::QtWidgets::MainWindow("NaviMainWindow", KDDockWidgets::MainWindowOption_HasCentralWidget)
	{}
~Navi();
...
QString GetInput(const QString &prompt, const QString &title,
...
inline void NextPage() noexcept { m_file_panel->NextPage(); }

inline void PrevPage() noexcept { m_file_panel->PrevPage(); }
...
void LogMessage(const QString &message,
                const MessageType &type = MessageType::INFO) noexcept;
...
inline std::string input(const std::string &prompt, const std::string &title,
                         const std::string &def) noexcept {

    QString result =
        GetInput(QString::fromStdString(prompt), QString::fromStdString(title),
...
inline std::string input_items(const std::string &prompt,
    ...
    return GetInput(QString::fromStdString(prompt),
                    ...
                    utils::stringListFromVector(items), default_choice - 1)
        .toStdString();
}
...
void SpawnProcess(const QString &command, const QStringList &args) noexcept;
...
void Lua__keymap_set(const std::string &key, const std::string &command,
...
inline std::string Get_default_directory() noexcept {
    return m_default_dir.toStdString();
}
...
inline void add_hook(const std::string &name,
                     const sol::function &func) noexcept {
...
inline bool add_bookmark(const std::string &bookmark_name,
                         const bool &highlight = false) noexcept {
    return m_bookmark_manager->addBookmark(
										   QString::fromStdString(bookmark_name),
										   QString::fromStdString(current_item_name()), highlight);
}

inline bool remove_bookmark(const std::string &bookmark_name) noexcept {
    return m_bookmark_manager->removeBookmark(
											  QString::fromStdString(bookmark_name));
}
...
inline std::string Get_copy_path_separator() noexcept {
    return m_copy_path_join_str.toStdString();
}
...
inline Statusbar::Module
create_statusbar_module(const std::string &name,
...
inline void create_toolbar_button(const std::string &name,
                                  const sol::table &opts) noexcept {
...
inline bool get_menubar_visible() noexcept { return m_menubar->isVisible(); }
...
inline bool get_toolbar_visible() noexcept { return m_toolbar->isVisible(); }
...
inline bool get_toolbar_icons_only() noexcept { return m_toolbar_icons_only; }
...
inline void
set_preview_panel_max_file_size(const std::string &max_file_size) noexcept {
...
inline void
set_preview_panel_image_dimension(const sol::table &table) noexcept {
...
inline bool get_menubar_icons() const noexcept { return m_menubar_icons; }

inline void set_cycle(const bool &state) noexcept {
    m_file_panel->SetCycle(state);
}

inline bool get_cycle() const noexcept { return m_file_panel->get_cycle(); }
...
inline void
set_visual_line_mode_padding(const std::string &padding) noexcept {
...
inline void copy_to() noexcept { m_file_panel->copy_to(); }

inline void move_to() noexcept { m_file_panel->move_to(); }

inline void link_to() noexcept { m_file_panel->link_to(); }
...
inline void set_vheader_foreground(const std::string &fg) noexcept {
    m_file_panel->tableView()->vheader()->set_foreground(
														 QString::fromStdString(fg));
}
...
inline void set_vheader_background(const std::string &bg) noexcept {
    m_file_panel->tableView()->vheader()->set_background(
...
inline bool get_vheader_visible() noexcept {
    return m_file_panel->tableView()->vheader()->isVisible();
}
...
inline bool get_file_panel_grid() noexcept {
    return m_file_panel->tableView()->showGrid();
}
...
QFutureWatcher<void> *m_thumbnail_cache_future_watcher =
...
KDDockWidgets::QtWidgets::DockWidget *m_preview_panel_dock =
    new KDDockWidgets::QtWidgets::DockWidget("Preview Panel");
KDDockWidgets::QtWidgets::DockWidget *m_file_panel_dock =
    new KDDockWidgets::QtWidgets::DockWidget("File Panel");
KDDockWidgets::QtWidgets::MainWindow *m_dock_container =
    new KDDockWidgets::QtWidgets::MainWindow("Container");
