...
int FileSystemModel::columnCount(const QModelIndex &parent) const {
    return parent.isValid() ? 0 : m_column_list.size(); // Three columns: Name, Size, Last Modified
}
...
QVariant FileSystemModel::data(const QModelIndex &index, int role) const {
    ...
    // if (role == Qt::BackgroundRole && isMarked) {
    //     return QBrush(QColor("#FFEB3B"));  // Light yellow background for marked files
    // }
...
void FileSystemModel::setColumnList(const QList<FileSystemModel::ColumnType> &cols) noexcept {
    m_column_list = cols;
}
...
QList<QModelIndex> FileSystemModel::match(const QModelIndex &start, int role, const QVariant &value,
