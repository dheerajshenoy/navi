...
void Navi::Error(const QString &reason) noexcept {
    ...
    m_statusbar->Message("HELLO WORLD", MessageType::ERROR);
}
...
void Navi::ExecuteExtendedCommand() noexcept {
    m_inputbar_completion->setMode("command");
    QString command = m_inputbar->getInput("Command");
    ProcessCommand(command);
    m_inputbar_completion->setMode("none");
}
...
void Navi::ShellCommandAsync(const QString &com) noexcept {
    ...
    if (com.isEmpty()) {
        bool ok;
        commandStr = QInputDialog::getText(this, "Shell Command Async",
...
void Navi::loadLayout(const std::string &layoutName) noexcept {
    ...
    if (savedLayouts.empty()) {
        QMessageBox::information(
                                 this, "Load layout",
                                 "No saved layouts detected. Please save a layout first to load them");
        return;
    }
    ...
    if (layoutName.empty()) {
        bool ok;
        QString selectedLayout = QInputDialog::getItem(
                                                       this, "Load a layout", "Choose a layout", savedLayouts, 0, false, &ok);
...
void Navi::initValidCommandsList() noexcept {
    ...
    m_commands = {
        ...
        {"bookmark-edit-path", {}},
        {"bookmark-go",
         {1, 1, {{[this]() { return m_bookmark_manager->getBookmarkNames(); }}}}},
        {"bookmarks-save", {}},
        {"new-file", {}},
        {"new-folder", {}},
        {"messages-pane", {}},
...
void Navi::initCompletion() noexcept {
    ...
    connect(m_inputbar_lineEdit, &LineEdit::tabPressed, m_inputbar_completion,
            &CompletionPopup::showPopup);

    connect(m_inputbar_lineEdit, &LineEdit::textChanged, m_inputbar_completion,
            [&](const QString &text) {
                if (text.isEmpty()) {
                    if (m_inputbar_completion->mode() == "command")
    ...
    m_inputbar_completion->setCompletions("command", m_commands.keys());
...
QStringList Navi::getCompletionsForCommand(const QString &command,
    ...
    const CompletionType &completionSource =
        m_commands[command].argCompletions[argIndex];
...
QStringList
Navi::traverse_table_iterative(const std::string &root_name) noexcept {
    ...
    while (!stack.empty()) {
        ...
        current_table.for_each([&](sol::object key, sol::object value) {
            if (key.is<std::string>()) {
                ...
                // If the value is a table, push it onto the stack for further
                // processing
